...
const layouts = {
  ...
  sevenOneTwo: {
    ...
    channels: [
      ...
      { label: "Lb", role: "Back L", azimuth: -145, elevation: 0, distance: 1.25 },
      { label: "Rb", role: "Back R", azimuth: 145, elevation: 0, distance: 1.25 },
      { label: "Ltm", role: "Top Middle L", azimuth: -90, elevation: 50, distance: 1.05 },
      { label: "Rtm", role: "Top Middle R", azimuth: 90, elevation: 50, distance: 1.05 },
    ],
  },
  sevenOneFour: {
    count: 12,
    channels: [
      ...
      { label: "Lb", role: "Back L", azimuth: -145, elevation: 0, distance: 1.25 },
      { label: "Rb", role: "Back R", azimuth: 145, elevation: 0, distance: 1.25 },
      { label: "Ltf", role: "Top Front L", azimuth: -45, elevation: 45, distance: 1.05 },
      { label: "Rtf", role: "Top Front R", azimuth: 45, elevation: 45, distance: 1.05 },
      { label: "Ltr", role: "Top Rear L", azimuth: -135, elevation: 45, distance: 1.1 },
      { label: "Rtr", role: "Top Rear R", azimuth: 135, elevation: 45, distance: 1.1 },
    ],
  },
};
...
function applyLayout(name) {
  ...
  if (name !== "custom" && preset.channels?.some((channel) => !channel.lfe && channel.elevation !== 0)) {
    dom.spaceMode.value = "3d";
  }
...
function updatePanner(index) {
  ...
  const pannerSettings = {
    ...settings,
    elevation: dom.spaceMode.value === "3d" || settings.lfe ? settings.elevation : 0,
  };
  const coords = sphericalToAudioPosition(pannerSettings);
...
function makeRange(index, key, label, min, max, step) {
  ...
  header.className = "compact-field-header";

  const labelEl = document.createElement("span");
  labelEl.textContent = label;

  const valueEl = document.createElement("output");
  valueEl.dataset.valueIndex = String(index);
  valueEl.dataset.valueKey = key;
  valueEl.textContent = formatControlValue(key, state.channelSettings[index][key]);
  ...
  input.addEventListener("input", () => {
    ...
    valueEl.textContent = formatControlValue(key, channel[key]);
    if (key === "gain") {
      updateChannelGain(index);
    } else {
      updatePanner(index);
...
function formatControlValue(key, value) {
  if (key === "gain") return `${Number(value).toFixed(2)} x`;
  if (key === "azimuth" || key === "elevation") return `${Math.round(Number(value))} deg`;
  if (key === "distance") return `${Number(value).toFixed(2)} u`;
  return String(value);
}
...
function syncSpaceMode() {
  ...
  state.channelSettings.forEach((_, index) => updatePanner(index));
  drawStage();
}
...
function drawStage() {
  ...
  state.channelSettings.forEach((settings, index) => {
    ...
    const heightChannel = isHeightChannel(settings);
    const radius = selected ? 18 : heightChannel ? 14 : 15;

    if (heightChannel && dom.spaceMode.value === "3d") {
      drawHeightGuide(point.x, point.y, settings.elevation);
    }

    ctx2d.fillStyle = settings.lfe ? "#d18b2e" : heightChannel ? "#7eaeff" : "#47b092";
    ctx2d.strokeStyle = selected ? "#ffffff" : "rgba(255,255,255,0.28)";
    ctx2d.lineWidth = selected ? 3 : 2;
    if (heightChannel) {
      drawDiamond(point.x, point.y, radius);
    } else {
      ctx2d.beginPath();
    ...
    const label = labelPosition(settings, point, cx, cy);
    if (heightChannel) {
      drawLeader(point.x, point.y, label.x, label.y);
    }
    drawLabel(settings.label, label.x, label.y, heightChannel);
  });
}

function isHeightChannel(settings) {
  return !settings.lfe && settings.elevation > 0;
}

function drawDiamond(x, y, radius) {
  ctx2d.beginPath();
  ctx2d.moveTo(x, y - radius);
  ctx2d.lineTo(x + radius, y);
  ctx2d.lineTo(x, y + radius);
  ctx2d.lineTo(x - radius, y);
  ctx2d.closePath();
}

function labelPosition(settings, point, cx, cy) {
  const angle = Math.atan2(point.x - cx, -(point.y - cy));
  const outward = isHeightChannel(settings) ? 70 : 25;
  const sideBias = Math.sign(Math.sin(angle)) * (isHeightChannel(settings) ? 18 : 0);
  return {
    x: point.x + Math.sin(angle) * outward + sideBias,
    y: point.y - Math.cos(angle) * outward,
  };
}

function drawHeightGuide(x, y, elevation) {
  const lift = Math.min(24, Math.max(10, elevation * 0.38));
  ctx2d.save();
  ctx2d.strokeStyle = "rgba(126,174,255,0.48)";
  ctx2d.lineWidth = 2;
  ctx2d.beginPath();
  ctx2d.moveTo(x, y + lift);
  ctx2d.lineTo(x, y - lift);
  ctx2d.stroke();
  ctx2d.setLineDash([4, 4]);
  ctx2d.strokeStyle = "rgba(126,174,255,0.28)";
  ctx2d.beginPath();
  ctx2d.ellipse(x, y + lift, 16, 6, 0, 0, Math.PI * 2);
  ctx2d.stroke();
  ctx2d.restore();
}
...
function drawLeader(fromX, fromY, toX, toY) {
  ctx2d.save();
  ctx2d.strokeStyle = "rgba(188,215,255,0.38)";
  ctx2d.lineWidth = 1;
  ctx2d.beginPath();
...
function drawLabel(text, x, y, isHeight) {
  ctx2d.save();
  ctx2d.font = "800 13px system-ui, sans-serif";
  ctx2d.textAlign = "center";
  ctx2d.textBaseline = "middle";
  const width = Math.ceil(ctx2d.measureText(text).width) + 12;
  const height = 19;
  ctx2d.fillStyle = isHeight ? "rgba(20,45,78,0.88)" : "rgba(32,38,36,0.82)";
  ctx2d.fillRect(x - width / 2, y - height / 2, width, height);
  ctx2d.strokeStyle = isHeight ? "rgba(188,215,255,0.48)" : "rgba(255,255,255,0.18)";
  ctx2d.strokeRect(x - width / 2, y - height / 2, width, height);
  ctx2d.fillStyle = "#f5f6f2";
...
function speakerToCanvas(settings, cx, cy, maxRadius) {
  ...
  const el = dom.spaceMode.value === "3d" ? (settings.elevation * Math.PI) / 180 : 0;
  const horizontalDistance = settings.distance * Math.cos(el);
  const normalizedDistance = Math.min(1, Math.max(0.08, horizontalDistance / 5));
...
function updateChannelInputs(index) {
  for (const key of ["azimuth", "distance", "elevation", "gain"]) {
    ...
    const value = dom.channelList.querySelector(`[data-value-index="${index}"][data-value-key="${key}"]`);
    if (value) value.textContent = formatControlValue(key, state.channelSettings[index][key]);
  }
}
