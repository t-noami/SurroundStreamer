...
- WASAPI/MMDevice endpoints are useful for mono/stereo inputs, but should not be assumed to expose surround capture unless that specific device has been verified.

### Linux

- Linux packages are not published for v0.1.1.
- Linux Audio Input capture is not implemented yet.
...
The macOS package was built and checked with the project release build process, including app signature verification, DMG verification, and packaged license checks.

The Windows package was built and checked with the project release build process, including packaged FFmpeg and license checks.
